...
def get_entropy(prob):
    entropy = torch.log(prob + 1e-6) * prob
    return -1 * entropy.sum().item()


def split_dataset(in_path, train_path, dev_path, test_path, dev_per=0.1, test_per=0.1):
    """
    split dataset into three part: [train, dev, test]
    :param in_path:
    :param train_path:
    :param dev_path:
    :param test_path:
    ...
    random.shuffle(data)
    data_len = len(data)
    dev_data_len = int(data_len * dev_per)
    test_data_len = int(data_len * test_per)
    train_data_len = data_len - dev_data_len - test_data_len

    dev_data = data[:dev_data_len]
    test_data = data[dev_data_len:(dev_data_len + test_data_len)]
    train_data = data[(dev_data_len + test_data_len):]

    if len(train_data) > 0:
        with open(train_path, 'w') as wf:
            json.dump(train_data, wf, indent=2)

    if len(dev_data) > 0:
        with open(dev_path, 'w') as wf:
            json.dump(dev_data, wf, indent=2)

    if len(test_data) > 0:
        with open(test_path, 'w') as wf:
            json.dump(test_data, wf, indent=2)

    return train_data_len, dev_data_len, test_data_len


def detect_unk_kb(data_path, out_path):
    with open(data_path, 'r') as f:
        doc_data = json.load(f)

    unk_docs = {}
    for d in tqdm(doc_data, desc='detecting unk kb...'):
        name = d['name']
        cur_k = [ele for ele in d['kb']]
        unk_k = set(AgentActs.ALL_SLOTS) - set(cur_k)

        for k in unk_k:
            if k not in unk_docs:
                unk_docs[k] = [name]
            else:
                unk_docs[k].append(name)

    # for k, v in unk_docs.items():
    #     print(k, len(v))

    with open(out_path, 'w') as wf:
        json.dump(unk_docs, wf, indent=2)


def detect_same_kb(data_path, out_path):
    with open(data_path, 'r') as f:
        doc_kb = json.load(f)
    ...
    # every document
    for d in tqdm(doc_kb, desc='detecting same kb...'):
        name = d['name']
        for slot, values in d['kb'].items():
            for v in values:
                if v not in same_docs[slot]:
                    same_docs[slot][v] = [name]
                else:
                    same_docs[slot][v].append(name)

    with open(out_path, 'w') as wf:
        json.dump(same_docs, wf, indent=2)
